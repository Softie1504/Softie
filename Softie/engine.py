...
class ClusterSegmentationWithYolo():
    def __init__(self, erode_size=5, erode_intensity=2):
        self.model = YOLO('yolov8n-seg.pt')
        self.erode_size = erode_size
        self.erode_intensity = erode_intensity
        self.background_image = cv2.imread("./static/music_logo2.jpg")

    def genrate_mask_from_result(self, results):
        for result in results:
            if result.masks:
                ...
                # get indices of results where class is a (people in COCO)
                ...
                erode_mask = cv2.erode(people_mask.cpu().numpy(), kernel, iterations=self.erode_intensity)   
    ...
    def apply_blur_with_mask(self, frame, mask, blur_strength=21):
        ...
        # Compare blurred and original frame
